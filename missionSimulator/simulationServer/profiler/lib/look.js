...
module.exports.start = function (port, host, options) {
	...
	var observerParameters = [];
		observerParameters.push(port);
		observerParameters.push(host);
		if (options.mongoSaving) {
			observerParameters.push(options.mongoSaving.host);
			observerParameters.push(options.mongoSaving.port);
			observerParameters.push(options.mongoSaving.db);
		}

	if (cluster.isMaster) {
		var observer = child_process.fork(__dirname + '/observer', observerParameters);
